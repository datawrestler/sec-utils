...
from secutils import FormIDX, SECContainer, DocumentDownloaderThread
...
def main():
    ...
    # parser.add_argument('--ciks', nargs='+', type=str, help='List of CIKs to download')
    # parser.add_argument('--cik_config', type=str, help='Path to cik config file')
    ...
    # TODO more elegant method for handling building directory structure
    for (yr, qtr) in tqdm(time, total=len(time)):
        ...
        files = FormIDX(year=yr, quarter=qtr, seen_files=seen_files, cache_dir=args.cache_dir, 
               form_types=args.form_types).index_to_files()
        if len(files) > 0:
